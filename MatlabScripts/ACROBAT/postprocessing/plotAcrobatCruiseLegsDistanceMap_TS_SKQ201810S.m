function plotAcrobatCruiseLegsDistanceMap_TS_SKQ201810S(top_dir, cruise_name)
...
latrng = [59.4  61.2];
short_thresh = 3;
gridded.dens = real(gridded.dens);
...
%         sgthin(cc,:) = naninterp( sgthin(cc,:));
         if (length(find(~isnan(sgthin(cc,:)))) >= 2)
            sgthin(cc,:) = naninterp( sgthin(cc,:));
        end
    end
...
%         dd =  find( sum( ~isnan( datin )) > short_thresh);
        ...
        for cc = 1:length(dds)
            ...
            datin( gridded.p > depthgrid(dds(cc)),cc) = nan;
        end
        if (ss == 1),
            temp = datin;
        elseif (ss == 2);
        ...
        if strcmp(cruise_name,'EcoFjordAcrobat2016')
            ...
            [c, d] = contour( v, gridded.p, real(sgthin) , 20:.1:30, '-', 'linewidth', 1, 'color', [1, 1, 1]*0);
        end
