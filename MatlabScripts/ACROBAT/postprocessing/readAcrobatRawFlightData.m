function readAcrobatRawFlightData( top_dir, cruise_name )

% function readAcrobatRawFlightData( top_dir, cruise_name )
%
% Load and process the raw flying data from the Acrobat during the deployment logged to
% a text file by the Sea Sciences flying software. Computer time and GPS
% time 
% Raw Data is also saved out as a .mat file to ./Data/ACROBAT/PROCESSED
%
% ACRO structure output [unit]
...
%  predefine the output matrix
 ...
 for d = 1:length(a)
     ...
     str2 = strcat( repmat( {'01/00/0000 '}, [size( cHHMMSS, 1 ), 1]), cHHMMSS );
%      timein = datenum(str2 , 'mm/dd/yyyy HH:MM:SS');
     timein = datenum(cHHMMSS); %  , 'mm/dd/yyyy HH:MM:SS');
     timein = timein - datenum(yy,00,00); 
...
%      dat.gpstime = dat.ctime+offsetR+offsetE; 
    offset = 0.2914;  % Calculated manually from data
    dat.gpstime = dat.ctime+offset; 
     ...
     for vv = 1:length( vars )
         ...
         out.(vars{vv})  = cat( 1, out.(vars{vv}) , dat.(vars{vv}) );
     end
