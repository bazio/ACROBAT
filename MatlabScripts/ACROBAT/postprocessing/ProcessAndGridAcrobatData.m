...
%this function completes the following tasks on the data output by
%ReadAcrobatRAW:
%1) data structures are read.

%2) unflagged gps time is collected, and tested for spikes. If less 10 spikes
% are detected, the spikes are removed and interpolated across.

%3) indicies for good time data are used to gather the rest of the data
%(salinity, temp, etc).

%4) A pressure file for ECO data is created from the CTD pressure record.
%Since the ECOpuck samples at a slower speed, this is accomplished via
%interpolation. 

%5) Calibration coefficients are applied to ECOpuck raw data.

%6) CTD files undergo temperature advancement, as suggested by SBE 49 manual.

%7) Salinity data run through a handcleaning function that allows users to
%create high thresholds, low thresholds, and polygon shapes, ect to
%identify unwanted data points. The good indices are stored as a
%variable.This handcleaning has the option via the redo and alter options
%to be run multiple times without the need to if a user for example removes some datapoints,
%and later discovers other data points that they want to remove.

%8) Turning points are identified, and plotted.This portion of code could be
%done much more cleanly, but it does work as is.


%9) The CTD data is gridded. In this case this specifically means that we create a
%matrix that is 66 rows (for each pressure bin 0:65) and X many columns for
%however many "profiles" are identified in the preceding step when we
%identified turning points. Each "profile" is a half yo of the ACROBAT.
%This means that columns do NOT represent distance bins, they are simply
%consective half yos. Each CTD variable (salinity, temp, density) has a
%data matrix. associated 1 D variables (Lat, long, time, distance) are
%contained in vectors.

%10) The ECOpuck data is gridded in the same manner as above. 

%11) ACOBAT data is saved out in a structure called gridded. The corrected 




% KIM 08.13
% ISAAC R 02.22 fixed an issue where turningpoints get out of sync if there
...
%also added a hand cleaning function.

% ECOpuck serial number
sn = 'FLBBCD2K-3883'; %must be changed to match the ECOpuck serial number used for the cruise.
...
%additional data points, redo must also be set to 1. This will load the previous version, apply it, and
...
combine=0;
%set this to 1 if during a cruise an insturment on the ACROBAT was swapped
%out with another. The example in the relevant code section (search for the
%second instance of combine) is for a case where two ECOpucks were used
%during a cruise. See section comments for details.
...
%%
%however, sometimes gpstime will get spikes.
...
%is a problem to be removed. This is done below.
dtmp2=(diff(tmp2));
...
if combine==1
    if cruise_name(1:10)=='SKQ202010S';
        ...
        %out from our orinal data set, the nans + the real values before
        %the date of interests and after the date of interest. 
...
[tt,in] = unique(t); %so if t no has no nans and has been extrolated and was the result of being unique this line is unneccessary.
...
%the end and then plus some nonreal bogus times that are connected to real bogus
...
%last real turningpoing doesn't have an end. Remember how all the lat
