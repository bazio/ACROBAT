...
%     py_df{ll} = boxcarsmooth( py_d(cols), 5);
%     if (ll==1),
%         py_dr = [py_df{ll}];
%     else
%         py_dr = [py_dr py_df{ll}]; 
%     end
% end
for in=1:N,
    up_bins = find(gridded.p < py_df(in));
    bot_bins = find(gridded.p >= py_df(in));
...
%position = [left bottom width height]
  ...
  a(3,1) = axes('position', [0.06      0.080   0.38 0.4004]);
  a(2,1) = axes('position', [0.55      0.5250  0.38 0.4004]);
  a(1,1) = axes('position', [0.06      0.5250  0.38 0.4004]);
  a(4,1) = axes('position', [0.55      0.080   0.38 0.4004]);
...
for in = 1:4,
    ...
    elseif (in == 4),
        h1 = m_scatter( gridded.lon, gridded.lat, 15, cbar(1,:),'filled');
        h8 = title( {[cruise_name];...
            [ ' Upper Layer Chlorophyll' ]},'fontsize', 16, 'fontweight', 'bold');
    end
    if (in ==4),
        caxis( lims{5})
        cb = colorbar;
        set( cb, 'fontsize', 12, 'fontweight', 'normal', 'ytick', cticks{5})
        ylabel( cb, units{5}, 'fontsize', 12,'fontname','Times')
    else,
        caxis( lims{in})
        cb = colorbar;
    ...
    m_ruler([0.2 0.8], .9);
    filename = [cruise_name, '_upper_layer_maps' ];
...
for in = 1:4,
    ...
    elseif (in == 4),
        h1 = m_scatter( gridded.lon, gridded.lat, 15, cbar(2,:),'filled');
        h8 = title( {[cruise_name];...
            [ ' Bottom Layer Chlorophyll' ]},'fontsize', 16, 'fontweight', 'bold');
    end
    if (in == 4),
        caxis( lims{5})
        cb = colorbar;
   ...
   % h4 = m_text(Site2(1)-0.1,Site2(2),'WAINWRIGHT','fontsize',6,'fontweight','bold','fontname','verdana', 'horizontalalignment', 'right');
    ...
    m_ruler([0.2 0.8], .9);
    filename = [cruise_name, '_lower_layer_maps' ];
    print('-dpng','-r300',[targetdir '/' filename]);
end
%%
if 0,
